...
def load_raw_covid_data(limit):
    ...
    for i, filename in enumerate(covid_images):
        try:
            image_path = os.path.join(covid_path, filename)
            image_npy = img.imread(image_path)  # Read the image
            positive = resize(
    ...
    #print(non_covid_images)
  
    for i, filename in enumerate(non_covid_images):
        try:
            image_path = os.path.join(non_covid_path, filename)
            image_npy = img.imread(image_path)
            negative = resize(
                image_npy, (IMG_SIZE, IMG_SIZE, 1), anti_aliasing=True)
    ...
    #    plt.imshow(negative[i:i+1].reshape(128, 128), cmap="gray")
    #    plt.title(f"Non-COVID Image {i}")  # Optional: Add a title to each plot
    #    plt.show()  # Explicitly show the image
